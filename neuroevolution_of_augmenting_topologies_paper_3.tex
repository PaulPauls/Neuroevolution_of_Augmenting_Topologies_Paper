...
Whilst Neuroevolution first gained significant traction in the research community with the NEAT algorithm (see section \ref{sec:neat}), as it was the first neuroevolution algorithm to address and solve fundamental shortcomings of prior research, have multiple considerable algorithms already been invented and widely spread. Representatives of those early algorithms are the SANE [cite] and ESP [cite] algorithms from Moriarty \& Gomez. These algorithms were already quite clever, exploring the ideas nowadays generally known as "cooperative coevolution" [cite], though the aforementioned significant shortcoming was that those algorithms were intended to only evolve Fixed-topology networks, particularly the weights in Fixed-topology networks. At the time this was a fruitful approach as modern methods of stochastic gradient descent like Backpropagation were not as wide spread and "Topology and Weight Evolving Neural Networks" (short form: TWEANNs) were not working as well as they could \href{https://www.oreilly.com/ideas/neuroevolution-a-different-kind-of-deep-learning}{[cite]}.

Until in 2002 "Neuroevolution of Augmenting Topologies" by Stanley\&Miikkulainen [cite] proposed novel enhancements to TWEANN algorithms, that significantly increased their viability (see section \ref{sec:neat_performance}). NEATs' main innovations were the ability to perform meaningful \textit{recombination} between two arbitrary neural networks with different topologies, effectively solving the 'Competing Conventions Problem' (see figure \ref{ill:comp_conv}), and furthermore to provide protection for topological innovation so that it does not disappear from the population prematurely. With the success of NEAT came variations of the algorithm - the most noteworthy of which are rtNEAT [cite] and odNEAT [cite]. rtNEAT introduced a real-time component into the algorithm, enabling faster and more flexible computation than with a generation-focused algorithm, while odNEAT introduces the ability to decentralize the algorithm and speed up parallelization significantly.

The field of neuroevolution was advanced further again significantly with the introduction of "HyperNEAT" by Risi\&Stanley [cite] in 2009. HyperNEAT proposes a NEAT like evolution of the genotype, but instead of the genotype representing the nodes and connections directly does the genotype represent a function that will generate the neural network. The encoding of the neural network therefore moved from \textit{direct} to \textit{indirect}.
